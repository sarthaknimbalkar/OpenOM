...
_LANDING_CSS = """
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .who{padding:0 0 30px}
    .who-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-top:34px}
    .pcard{display:block;text-decoration:none;color:var(--ink);background:var(--paper);border:1px solid var(--rule);border-radius:12px;padding:22px 20px;transition:transform .12s ease,box-shadow .12s ease}
    .pcard:hover{transform:translateY(-2px);box-shadow:0 14px 28px -18px rgba(15,29,48,.45)}
    .pcard h4{font-size:17px;font-weight:800;letter-spacing:-.01em}
    .pcard p{margin-top:8px;font-size:14px;color:var(--ink-soft)}
    .pcard .go{display:inline-block;margin-top:14px;font-family:var(--mono);font-size:12px;color:var(--link)}
    @media(max-width:920px){.who-grid{grid-template-columns:1fr 1fr}}
...
<pre>{
  ...
  <section class="who"><div class="wrap">
    ...
    <h3>Four ways in.</h3>
    <div class="who-grid">
      <a class="pcard" href="/docs/quickstart-broker"><h4>Brokers &amp; authors</h4>
        <p>You have OM PDFs and want them to carry structured, verifiable data - visually identical output.</p>
        <span class="go">Broker quick-start &rarr;</span></a>
      <a class="pcard" href="/docs/quickstart-portal"><h4>Portals &amp; consumers</h4>
        <p>Read and trust openOM data on OMs you host or receive, with an honest trust badge.</p>
        <span class="go">Portal quick-start &rarr;</span></a>
      <a class="pcard" href="/docs/quickstart-developer"><h4>Developers</h4>
        <p>Build against the standard: JSON Schema, JSON-LD context, Python + TypeScript.</p>
        <span class="go">Developer quick-start &rarr;</span></a>
      <a class="pcard" href="/docs/grounding-ai"><h4>AI builders</h4>
        <p>Ground your CRE agent in verified facts via MCP instead of re-parsing PDFs.</p>
        <span class="go">Grounding AI &rarr;</span></a>
    </div>
  </div></section>
  ...
  <section class="surf" id="surfaces"><div class="wrap">
    ...
    <div class="surf-grid">
      <div class="card2 mcp reveal">
        ...
        <p class="sub">Six deterministic tools. No inference on our side - your model, your subscription. Free and self-hostable.</p>
