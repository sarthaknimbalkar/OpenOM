...
describe("[OM-CANON-015/019] JCS number formatting torture", () => {
  ...
  test("exponent thresholds match ECMAScript Number::toString", () => {
    expect(jcs({ x: 1e-7 })).toBe('{"x":1e-7}');
    expect(jcs({ x: 1e21 })).toBe('{"x":1e+21}');
  });

  test("just inside the thresholds stays in plain decimal notation", () => {
    expect(jcs({ x: 1e-6 })).toBe('{"x":0.000001}');
    expect(jcs({ x: 1e20 })).toBe('{"x":100000000000000000000}');
  });
