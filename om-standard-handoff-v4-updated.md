...
- **[OM-DoD-003] M2 schema gate.** `/spec/om-0.1.schema.json` (2020-12) published; `om_validate` returns errors that block and warnings that don't ([OM-ERR-001]); every §B vector reproduces its expected code set exactly ([OM-VEC-003]); per-field `source` tags and the reserved `signature` field (rejected via OMV-E003 if populated) are enforced. *Verify:* `pytest spec/tests/test_vectors.py`.
...
- **[OM-DoD-006] M5a consumer-mode gate.** `/js` read/validate passes the cross-impl test against pikepdf ([OM-VEC-002]); MV3 detection re-fetches viewed PDFs (never scrapes the viewer) and drives the badge through all §AA states on labeled fixtures; the payload card renders; domain-origin verification meets the §10.1 definition; named-webhook publish sends an HMAC-signed, replay-protected envelope (§5b) with a working test-fire; **no inference dependency present in the consumer bundle** (import/graph assertion). *Verify:* `npm --prefix js test && npm --prefix extension test:consumer && node scripts/assert-no-inference.js extension/dist/consumer`.
- **[OM-DoD-007] M5b author-mode gate.** Download interception (Buildout + custom domains) → review panel → validate → Assert & Embed via `/js`; local extraction via Prompt API with a **network-egress assertion of 0 bytes** during extraction ([OM-PRIV-001]); hosted path stubbed behind §15 Q2; a broker completes an embed without touching the CLI. *Verify:* `npm --prefix extension test:author` (includes the egress assertion).
...
- **[OM-DD-025] `lease.options[]` (Option object).** Renewal/extension options. Each element:
  ...
  An Option element MAY carry a `source` tag ([OM-DD-004]). When both `escalation` (prose) and `escalationRate` (machine) are present they SHOULD agree; disagreement is advisory only.
