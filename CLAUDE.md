...
**Since M5 - launch/go-public sweep (2026-08-22).**
- **Live site at openom.app.** `site/` (generated by `spec/scripts/gen_site.py` + `gen_docs.py`,
  drift-locked by `spec/tests/test_site.py`) is deployed to Cloudflare Pages. Full design system (the
  "one file, two readers" mock): landing (hero, split human-OM/machine-JSON, math strip, how-it-works,
  four-persona "who it's for", two-surfaces, trust, FAQ) + all `/docs/*` + `/verify/` share it. A real
  embedded **sample OM** (`spec/assets/openom-sample.pdf`) is downloadable and drops into `/verify/`.
...
- **Hosted MCP** is self-hosted on the **gx10** box (`om-mcp-http` + `cloudflared` systemd services)
  and **access-gated** via Cloudflare Access (owner email + service token) - NOT an open endpoint. The
  public is pointed at the free local stdio server. See the auto-memory + DEPLOY.md.
...
- **Repo made public-ready.** Internal strategy docs (recon, decision memo, the old normative/handoff
  ...
  bumps). CI-parity gates green.

**Next: adoption.** Supply is the bottleneck - seed embedded OMs at the source (the activated Buildout
pull, a platform, or a bulk back-catalog embed). Consider a public read-only MCP on proper infra
(Cloudflare Worker) rather than the home box; a README + `examples/` polish for the public launch; and
the sole paid product (hosted inference extraction, a separate Vervelio build).
