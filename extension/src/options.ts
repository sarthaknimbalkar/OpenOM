...
export function renderOptions(root: HTMLElement, v: OptionsView): void {
  ...
  set.append(
    ...
    field("Badge openOM links on listing pages", toggle("o-linkbadging", v.settings.linkBadging)),
  );
...
if (typeof chrome !== "undefined" && chrome.storage?.local) {
  void (async () => {
    ...
    root.querySelector("#save")?.addEventListener("click", async () => {
      ...
      await setSettings({ linkBadging: checked("o-linkbadging"), proactiveDetection: checked("o-proactive") });
