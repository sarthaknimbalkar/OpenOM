...
test("[OM-PRIV-001] extraction makes ZERO network requests leave the device", async ({ context, extensionId }) => {
  ...
  await expect(page.locator('input[data-path="/deal/capRate"]')).toHaveValue("0.0575"); // extraction ran
...
test("on-device extraction pre-fills the review, then the human asserts", async ({ context, extensionId }) => {
  ...
  const page = await openPanel(context, extensionId, `${BROKER}/author/text.pdf`);
  await page.click(".extract-btn");
  await expect(page.locator('input[data-path="/deal/capRate"]')).toHaveValue("0.0575"); // form pre-filled
...
test("#93 noiType/noiAsOfDate controls appear once deal.noi is entered", async ({ context, extensionId }) => {
  const page = await openPanel(context, extensionId, `${BROKER}/author/plain.pdf`);
...
test("editing core fields via the form → assert → read-back", async ({ context, extensionId }) => {
  const page = await openPanel(context, extensionId, `${BROKER}/author/plain.pdf`);
  await fillProfile(page);
  await page.fill('input[data-path="/deal/capRate"]', "0.06");
  await page.fill('input[data-path="/property/address/streetAddress"]', "1000 Example Rd");
  await expect(page.locator("#assert")).toBeEnabled();
  await page.click("#assert");
  const read = await readDownloadedPayload(page);
  const p = read.payload as Record<string, Record<string, unknown>>;
  expect(p.deal.capRate).toBe(0.06);
  expect((p.property.address as Record<string, unknown>).streetAddress).toBe("1000 Example Rd");
});

test("#95 adding a rent-schedule row shows it in the finalized preview", async ({ context, extensionId }) => {
  const page = await openPanel(context, extensionId, `${BROKER}/author/plain.pdf`);
  await page.click(".add-rent");
  await page.fill('input[data-path="/lease/rentSchedule/0/annualRent"]', "123456");
